...
<style scoped></style>
 ...
 const categoryStore = useCategoryStore();
 const router = useRouter();
 const newRecipe = ref({ title: "", type: "", ingredient: "", category_id: "" }); // Changement à `category_id`
 const recipeId = Number(router.currentRoute.value.params.id);
   ...
   try {
     ...
     if (recipeLoaded) {
       newRecipe.value = {
         ...
         type: recipeLoaded.type,
         ingredient: recipeLoaded.ingredient,
         category_id: recipeLoaded.category_id, // Changement à `category_id`
       };
     }
   } catch (error) {
     console.error("Erreur lors du chargement des données :", error);
 ...
 const saveRecipe = async () => {
   if (
     ...
     !newRecipe.value.type ||
     !newRecipe.value.ingredient ||
     !newRecipe.value.category_id // Changement à `category_id`
   ) {
     alert("Veuillez remplir tous les champs !");
     return;
   }
   ...
   <div class="container mt-5">
     ...
     <div v-else class="p-4 bg-light rounded shadow-sm">
       ...
       <div class="input-group mb-4">
         ...
         <select class="form-select" v-model="newRecipe.category_id"> <!-- Changement à `category_id` -->
           <option
             v-for="category in categoryStore.categories"
             :key="category.id"
             :value="category.id"
       ...
       <button
         ...
         "
       >
         <i class="fas fa-save"></i> {{ t("recipes.edit_form.save") }}
       </button>
     </div>
   </div>
 </template>
