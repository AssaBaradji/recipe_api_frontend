...
const router = useRouter(); // Correction de la variable `route` à `router`
const recipeStore = useRecipeStore();
const categoryStore = useCategoryStore();
const { t } = useI18n();
...
const newRecipe = ref({
  ...
  type: "",
  ingredient: "",
  category_id: "", // Assurez-vous de garder `category_id` ici
});
...
const submitRecipe = async () => {
  ...
  try {
    await recipeStore.addRecipeToAPI(newRecipe.value);
    successMessage.value = t("recipes.addForm.success");
    router.push({ name: "recette" }); // Correction de `route` à `router`
  } catch (error) {
    errorMessage.value = t("recipes.addForm.error");
  }
};
